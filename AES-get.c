...
{
    //API KEY: 470c17efe693dd73
    //SECRET:  269e1c8d986ae8d3
#if 0
    if (3 != argc)
    {
        printf("Usage: \n");
	...
	if (0 == ini.open("/root/openwrt_widora/package/wzrecord/src/ver.ini"))
	{
	...
	if (0 == ini.open("/root/openwrt_widora/package/wzrecord/src/ver.ini"))
	{
        ini.read_string("base", "app_key", "ccccc", appkey_encrypt, sizeof(appkey_encrypt) - 1);
        ini.read_string("base", "app_secret", "ccccc", appsecret_encrypt, sizeof(appsecret_encrypt) - 1);
		ini.close();
	}

    int appkey_encrypted_len = strlen(appkey_encrypt);
    int appsecret_encrypted_len = strlen(appsecret_encrypt);
    printf("strlen (appkey_encrypt) = %d\n", appkey_encrypted_len);
    printf("strlen (appsecret_encrypt) = %d\n", appsecret_encrypted_len);

    char appkey_text[256] = {0};
    char appsecret_text[256] = {0};
    
    WzDecrypt(appkey_encrypt, appkey_text, appkey_encrypted_len, key);
    WzDecrypt(appsecret_encrypt, appsecret_text, appsecret_encrypted_len, key);
    printf("after decrypt, appkey_text = %s\n", appkey_text);
    printf("after decrypt, appsecret_text = %s\n", appsecret_text);
